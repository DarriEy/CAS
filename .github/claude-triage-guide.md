# CI Triage Guide — CAS (Community Attribute Service)
...
CAS connectors extract **geospatial attributes** (DEM, soil, land cover, climate, …) for a
geometry and return **zonal statistics in a canonical `AttributeResult`** (value, units, quality
flags, coverage fraction, pixel count). A connector wraps one upstream data provider/protocol
(WCS, STAC+COG, OPeNDAP, Zarr) and ends at that canonical result.
...
| `real_bug` | A genuine logic error in non-adapter CAS code. | **Report only** (describe the fix) |
| `other` | You cannot confidently classify it. | **Report only** |

## The canonical contract — never auto-fixed

Editing anything under `src/cas/core/` is a `contract_change` (human-only), never drift:
- `models.py` — `Dataset`, `AttributeResult` (value, units, `coverage_fraction`, `pixel_count`),
  quality flags (GOOD, SUSPECT, PARTIAL, MISSING, DEGRADED, ESTIMATED).
- `BaseConnector` and the protocol mixins (`connectors/base.py`, `connectors/protocols/`) — the
  public connector interface.

## The scope rule (critical — read before opening any fix PR)

An `adapter_drift` / `data_drift` fix **must change only files under `src/cas/connectors/` or
`tests/`**. If the minimal fix would touch **any** other path — `pyproject.toml`, `.github/`,
`src/cas/core/`, docs, packaging — then it is **not** adapter/data drift. Reclassify:
- touches `src/cas/core/` (or `BaseConnector`) → `contract_change`
...
pytest tests/ -v --tb=short -m "not network" --cov=cas
```
