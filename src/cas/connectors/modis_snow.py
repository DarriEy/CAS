...
"""MODIS Snow Cover connector — global 500m snow extent via Planetary Computer STAC+COG."""
...
SNOW_VAR = Variable(
    name="snow_cover", units="%", data_type=DataType.CONTINUOUS,
    valid_range=(0, 100),
    description="Maximum snow cover extent over 8-day period",
)


@register("modis_snow")
class MODISSnowConnector(STACMixin, BaseConnector):
    slug = "modis_snow"
    display_name = "MODIS Snow Cover 500m"
    base_url = STAC_URL
    protocol = "stac_cog"
    ...
    async def list_datasets(self) -> list[Dataset]:
        return [
            Dataset(
                id=f"{self.slug}:snow_cover",
                provider=self.slug,
                name="MODIS 500m Snow Cover (8-day)",
                description="Global 500m 8-day maximum snow extent — controls snowmelt timing and volume",
                variables=[SNOW_VAR],
                resolution_m=500,
                crs="EPSG:4326",
    ...
    async def extract(
        ...
        if not items:
            return AttributeResult(
                dataset_id=dataset_id, variable="snow_cover", value=None,
        ...
        asset_key = "Maximum_Snow_Extent"
        if asset_key not in item.get("assets", {}):
            asset_key = "Eight_Day_Snow_Cover"
        if asset_key not in item.get("assets", {}):
        ...
        return AttributeResult(
            dataset_id=dataset_id, variable="snow_cover", value=value,
            units="%", aggregation=AggregationMethod.MEAN,
