...
"""NASADEM connector — global 30m elevation (SRTM successor) via STAC + COG."""
...
@register("nasadem")
class NASADEMConnector(STACMixin, BaseConnector):
    slug = "nasadem"
    display_name = "NASADEM 30m (SRTM)"
    base_url = STAC_URL
    protocol = "stac_cog"
    ...
    async def list_datasets(self) -> list[Dataset]:
        return [
            Dataset(
                ...
                provider=self.slug,
                name="NASADEM 30m Elevation",
                description="Global 30m DEM derived from SRTM with void-filling (NASADEM HGT v001)",
                variables=[ELEVATION_VAR],
                resolution_m=30,
                crs="EPSG:4326",
                bbox=BoundingBox(min_lon=-180, min_lat=-56, max_lon=180, max_lat=60),
                temporal=TemporalExtent(temporal_type=TemporalType.STATIC),
                protocol=Protocol.STAC_COG,
                license="Public Domain",
                citation="NASA JPL, NASADEM HGT v001",
            )
        ]
