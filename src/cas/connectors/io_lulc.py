...
"""Impact Observatory 10m Annual LULC via Planetary Computer STAC+COG."""
...
LC_VAR = Variable(
    ...
    description="Impact Observatory 10m LULC (9 classes, Sentinel-2 derived)",
)


@register("io_lulc")
class IOLULCConnector(STACMixin, BaseConnector):
    slug = "io_lulc"
    display_name = "Impact Observatory 10m LULC"
    base_url = STAC_URL
    protocol = "stac_cog"
    ...
    async def list_datasets(self) -> list[Dataset]:
        return [Dataset(
            ...
            provider=self.slug,
            name="IO 10m Annual Land Use/Land Cover",
            description="Global 10m LULC from Sentinel-2 (Impact Observatory / Esri)",
            variables=[LC_VAR],
            resolution_m=10,
            crs="EPSG:4326",
            ...
            license="CC-BY-4.0",
            citation="Karra et al. 2021, Impact Observatory / Esri 10m LULC",
        )]
    ...
    async def extract(
        ...
        if isinstance(value, dict):
            value = {IO_CLASSES.get(int(k), f"class_{k}"): v for k, v in value.items()}
