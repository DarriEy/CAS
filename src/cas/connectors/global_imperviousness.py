...
"""Global impervious surface — built-up area and imperviousness via OpenLandMap COGs."""
...
IMPERV_LAYERS = {
    "built_up_area": {
        ...
        "variable": Variable(
            name="built_up_fraction", units="%",
            data_type=DataType.CONTINUOUS, valid_range=(0, 100),
            description="Built-up area fraction from ESA CCI (2018)",
        ),
    },
    "nightlights": {
        "url": f"{OLM_BASE}/layers1km/lcv_nightlights_viirs.dmsp_m_1km_s0..0cm_2013..2019_v1.0.tif",
        "variable": Variable(
            name="nightlights", units="nW/cm2/sr",
            data_type=DataType.CONTINUOUS, valid_range=(0, 10000),
            description="Mean nighttime light intensity (VIIRS/DMSP composite)",
        ),
    },
    "population_density": {
        "url": f"{OLM_BASE}/layers1km/pop_count_landscan_m_1km_s0..0cm_2019_v1.0.tif",
        "variable": Variable(
            name="population_density", units="persons/km2",
            data_type=DataType.CONTINUOUS, valid_range=(0, 1000000),
            description="LandScan ambient population density (2019)",
        ),
    },
}
...
class GlobalImperviousnessConnector(STACMixin, BaseConnector):
    ...
    async def list_datasets(self) -> list[Dataset]:
        datasets = []
        for ds_key, ds_info in IMPERV_LAYERS.items():
            ...
            datasets.append(Dataset(
                ...
                citation="OpenLandMap; ESA CCI Built-up; LandScan",
            ))
        return datasets
    ...
    async def extract(
        ...
        if layer_key not in IMPERV_LAYERS:
            raise DataFormatError(self.slug, f"Unknown layer: {layer_key}")

        ds_info = IMPERV_LAYERS[layer_key]
