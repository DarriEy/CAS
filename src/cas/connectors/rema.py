...
"""REMA connector — Reference Elevation Model of Antarctica 8m via PGC STAC + COG."""
...
@register("rema")
class REMAConnector(STACMixin, BaseConnector):
    slug = "rema"
    display_name = "REMA Antarctica 8m"
    ...
    async def list_datasets(self) -> list[Dataset]:
        return [
            Dataset(
                ...
                provider=self.slug,
                name="REMA Mosaic 8m Elevation",
                description="Reference Elevation Model of Antarctica 8m from PGC",
                variables=[ELEVATION_VAR],
                resolution_m=8,
                crs="EPSG:3031",
                bbox=BoundingBox(min_lon=-180, min_lat=-90, max_lon=180, max_lat=-53),
                temporal=TemporalExtent(temporal_type=TemporalType.STATIC),
                protocol=Protocol.STAC_COG,
                license="Open (PGC)",
                citation="Howat et al., REMA, Polar Geospatial Center",
            )
        ]
